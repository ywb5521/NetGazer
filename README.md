...
| libpcap | — | Packet capture (`apt install libpcap-dev`) |
| protoc | — | Proto compilation (`apt install protobuf-compiler`) |
...
# Backend
cd backend
go build -o bin/netgazer-server ./cmd/server
go build -o bin/netgazer-agent ./cmd/agent
...
```bash
./backend/bin/netgazer-server \
  --http-port 8080 \
  --grpc-port 50051 \
  --db ./netgazer.db
```

On first access, visit `http://localhost:8080/setup` to create an admin account.

**Step 2 — Start agents on monitored hosts**

```bash
sudo ./backend/bin/netgazer-agent \
  --server-addr <server-ip>:50051 \
  --interfaces eth0 \
  --node-id datacenter-1 \
  --tags production,rack-a
```

Packet capture requires root or capabilities:

```bash
sudo setcap cap_net_raw,cap_net_admin=eip ./bin/netgazer-agent
```
...
| `--db` | `./gtopng.db` | SQLite database path |
...
# Start server (Dashboard :8080, gRPC :50051)
docker compose up -d
...
# volumes:
#   - ./geoip:/var/lib/netgazer/geoip
```

Dashboard: `http://localhost:8080` (first visit → `/setup` to create admin account).

Production with Nginx reverse proxy on port 80:
...
```nginx
# /etc/nginx/sites-available/netgazer

server {
    listen 9527;
    server_name _;

    # gRPC proxy for agent connections
    location /gtopng.v1.AgentService/ {
    ...
    # Static frontend
    location /gtopng/ {
        alias /var/www/gtopng/;
        try_files $uri /gtopng/index.html;
    }
}
```
...
```ini
# /etc/systemd/system/netgazer-server.service
[Unit]
Description=NetGazer Server
After=network.target

[Service]
Type=simple
ExecStart=/opt/netgazer/bin/netgazer-server \
  --http-port 8080 \
  --grpc-port 50051 \
...
│   └── vite.config.ts
├── proto/
│   └── gtopng/v1/agent.proto     # gRPC service definition
├── deploy/
│   └── nginx/gtopng.conf         # Nginx reverse proxy config
...
**环境要求**: Go 1.23+、Node.js 22+、libpcap、protoc
...
# 启动服务端
./backend/bin/netgazer-server --http-port 8080 --grpc-port 50051
...
首次访问 `http://localhost:8080/setup` 创建管理员账户，然后登录即可使用。
...
**Docker（推荐）**：`docker compose up -d` 一键启动，Dashboard → `http://localhost:8080`。

**裸机部署**：提供 Nginx 反向代理配置（[deploy/nginx/gtopng.conf](deploy/nginx/gtopng.conf)）和 systemd 服务示例，支持生产环境部署。详见上方英文部署章节。
