...
export function TopTalkersCard() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm font-medium">{t.dashboard.topTalkers}</CardTitle>
      </CardHeader>
      <CardContent>
        {topTalkers.length === 0 ? (
            ...
            <TableBody>
              {topTalkers.map((h) => (
                <TableRow key={h.ip}>
                  ...
                  <TableCell className="text-xs">{h.hostname || h.vendor || '-'}</TableCell>
                  <TableCell className="text-xs text-right font-medium text-emerald-500">
                    {formatBytes(h.bytes_per_sec)}/s
                  </TableCell>
