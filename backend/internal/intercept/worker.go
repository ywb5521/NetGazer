...
type workerPacket struct {
	...
	SetVerdict func(Verdict, []byte) error
}
...
func (w *worker) handle(streamID uint32, p gopacket.Packet) (Verdict, []byte) {
	...
	case *layers.UDP:
		...
		if v == VerdictAcceptModify && modPayload != nil {
...
func (w *worker) handleTCP(ipFlow gopacket.Flow, pMeta *gopacket.PacketMetadata, tcp *layers.TCP) Verdict {
	ctx := &tcpContext{
...
func (w *worker) handleUDP(streamID uint32, ipFlow gopacket.Flow, udp *layers.UDP) (Verdict, []byte) {
	ctx := &udpContext{
		Verdict: udpVerdictAccept,
	}
	w.udpStreamManager.MatchWithContext(streamID, ipFlow, udp, ctx)
	return Verdict(ctx.Verdict), ctx.Packet
}
